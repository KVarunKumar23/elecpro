...
const TOPICS_L2 = [
...

  {
    ...
    beginner:{
      ...
      theory:"[IS]\nEARTHING SYSTEM TYPES (IS / IEC):\n\nTN-S: Separate neutral (N) and protective earth (PE) conductors throughout. Clean separation prevents noise. Recommended for new installations.\n\nTN-C-S: PEN combined from source to main board, then split into separate N and PE. Common in India.\n\nTT: Separate earth electrode at installation. Requires RCD as primary protection (earth fault current is low).\n\nIT: No direct earth connection. First fault causes very low fault current. Insulation monitoring mandatory. Used in hospitals.\n\nEARTHING ELECTRODES & SOIL (IS 3043):\nPipe: 25mm dia × 3m GI or Copper (standard for general use).\\nPlate: 600×600×3mm Cu or 600×600×6mm GI (high fault currents).\nStrip: horizontal buried copper strip 25×1.6mm min (rocky soils).\n\nSOIL RESISTIVITY GUIDE:\nAlluvial/Clay: 10–50 Ω·m (excellent)\nLoam/Arable: 50–100 Ω·m (good)\nGravel/Sand: 1000–5000 Ω·m (requires treatment/deep driven)\nSolid Rock: >10000 Ω·m (requires buried strips/chemical)\n\n[IEC]\nEARTHING SYSTEM TYPES (IEC 60364):\n\nTN-S: Separate neutral (N) and protective earth (PE) conductors throughout. Clean separation prevents noise.\n\nTN-C-S: PEN combined from source to main panel, then split into N and PE.\n\nTT: Separate earth electrode at installation. Requires RCD for fault protection.\n\nIT: No direct earth connection. First fault causes very low fault current. Insulation monitoring mandatory. Used in hospitals / hazardous areas.\n\nEARTHING ELECTRODES:\nRod electrodes: 16mm dia × 2.4m copper\nFoundation earth: bare conductor embedded in concrete footings\nMesh/grid: buried interconnected conductors\n\n[NEC]\nGROUNDING SYSTEM TYPES (NEC):\n\nSolidly Grounded System: The neutral is intentionally connected to ground with no added impedance. Most common for LV commercial/residential. High fault current allows breakers to trip quickly.\n\nHigh-Resistance Grounded (HRG): A resistor limits ground-fault current to <10A. System continues running on first fault. Prevents arc flash hazards. Used in industrial 480V systems.\n\nUngrounded System: No intentional ground connection. Ground detectors required. Very low fault current on first fault, but high transient overvoltages possible.\n\nSeparately Derived System: A premises wiring system whose power is derived from a source of electric energy or equipment other than a service (e.g., transformer). Requires its own system bonding jumper.\n\nGROUNDING ELECTRODES (NEC 250.52):\nGround Rods: 5/8 inch dia × 8 ft copper-clad steel (minimum two rods usually required)\nConcrete-Encased (Ufer): 20 ft of rebar or bare copper in foundation bottom\nGround Ring: Bare copper wire encircling the building\nMetal Underground Water Pipe: Requires supplemental electrode",
      formula:{
      ...
      example:{
        sector:'ind',
        given:'Industrial building: soil ρ = 80 Ω·m. [[IS 3043 requires ≤ 5Ω|NEC requires ≤ 25Ω]]. Electrode: L=[[3m|8ft (2.44m)]], d=[[25mm|5/8in (16mm)]].',
        steps:[
          'R_single = 80/(2π×[[3|2.44]]) × [ln(4×[[3/0.025|2.44/0.016]]) - 1]',
          '[[= 80/18.85 × [ln(480) - 1]|= 80/15.33 × [ln(610) - 1]]]',
          '[[= 4.244 × [6.17 - 1]|= 5.219 × [6.41 - 1]]]',
          '[[= 4.244 × 5.17 = 21.9 Ω (single electrode)|= 5.219 × 5.41 = 28.2 Ω (single rod)]]',
          'Electrodes in parallel (with 0.7 coupling): n × R_parallel = R_single/(n × 0.7)',
          'Need n: R_single/(n × 0.7) ≤ [[5|25]] → [[n ≥ 21.9/(5 × 0.7) = 6.26 → 7 electrodes|n ≥ 28.2/(25 × 0.7) = 1.61 → 2 rods]]',
          'Check: [[21.9/(7 × 0.7) = 4.47 Ω ≤ 5 Ω ✓|28.2/(2 × 0.7) = 20.1 Ω ≤ 25 Ω ✓]]'
        ],
        result:'[[7 pipe electrodes in parallel → ~4.5 Ω. Connect with copper strip|2 ground rods in parallel → ~20 Ω. Connect with bare copper conductor]]. Test before commissioning.'
      },
      rot:["Rule of thumb: each [[3m|8ft]] pipe electrode in normal soil (100 Ω·m) gives ~[[20–30 Ω|25-35 Ω]]. Need ≤ [[5Ω|25Ω]]? Plan for [[5–8|2]] electrodes in parallel minimum.","For soil resistivity > 500 Ω·m (rocky/sandy): chemical treatment of soil around electrodes OR deep-driven electrodes OR foundation earth required."],
      mistakes:["Using only one earth electrode and assuming it meets [[IS 3043|NEC]] — single electrode in normal soil is typically [[20–30|25-35]] Ω, far above the [[5Ω|25Ω]] limit","Not testing earth resistance after installation — calculation is only an estimate; actual soil conditions vary. Always measure with earth tester.","[[Connecting the neutral and earth together at a remote sub-distribution board in a TN-S system — destroys the TN-S and creates dangerous circulating currents|Connecting the neutral and ground together downstream of the main service disconnect — destroys the single-point ground and creates dangerous circulating currents]]"],
      interviewQs:["[[What is the difference between TN-S and TT earthing systems?|Why do we bond the neutral to ground only at the main service?]]","[[Why is TN-C prohibited in new installations?|What is the purpose of an equipment grounding conductor?]]","[[When would you specify an IT earthing system?|When would you use a High-Resistance Grounding (HRG) system?]]"],
...

  {
    ...
    beginner:{
      ...
      theory:"WHAT A PANEL SCHEDULE CONTAINS:\nFor each circuit in the DB:\n— Circuit number\n— Description (what it feeds)\n— Phase (R, Y, B for 3-phase DB)\n— Breaker rating and type (MCB B/C/D, MCCB)\n— Pole count (1P, 2P, 3P)\n— Load (kW, kVA, or amps)\n— Cable size (mm²)\n— Cable length (m)\n— Voltage drop (%)\n— Remarks (future, spare, interlock, etc.)\n\n[IS]\nSERVICE ENTRANCES & BRANCH CIRCUITS (IS 732):\nClearances: Minimum clear working space in front of DBs (typically 1m deep). No storage allowed in front of panels.\nOverloading limits: IS 732 bounds lighting circuits to max 10 points or 800W. Power circuits limited to max 2 points or 3000W.\nPhase spacing: Separate phases must be separated by at least 2m indoors to prevent phase-to-phase shock under fault, or prominently labelled.\n\n[NEC]\nSERVICE ENTRANCES & BRANCH CIRCUITS (NEC):\nClearances: NEC 110.26 requires 3-foot depth, 30-inch width clearance in front of panels.\nOverloading limits: Breaker shall not be loaded beyond 80% for continuous loads (Art 210.20).\nPhase spacing: Multiwire branch circuits must have simultaneous disconnects for all ungrounded conductors (Art 210.4).\n\n[IEC]\nSERVICE ENTRANCES & BRANCH CIRCUITS (IEC 60364):\nClearances: Minimum access for operation and maintenance. No storage allowed.\nOverloading limits: Circuit design current (Ib) must not exceed protective device rating (In).\nPhase spacing: Clear segregation of circuits; neutral conductors must be routed with their respective phase conductors.\n\n[ALL]\nPHASE BALANCING:\nIn a 3-phase DB, single-phase circuits are distributed across R, Y, B phases.\nTarget: phase currents within 10% of each other.\nNeutral carries unbalanced current.\nCommon pattern: lighting on Phase R, power on Phase Y, AC on Phase B.\n\nSPARE CAPACITY:\nBest practice: leave 20–25% of breaker ways spare for future circuits.\nAlso: breaker rating × 0.8 = maximum connected load (loading factor).\n\nDB TOTAL LOAD:\nSum all circuit loads, apply demand factor, convert to kVA.\nMDB circuit breaker for this DB = calculated kVA / (√3 × 0.415) = Amps.",
      formula:{
...

  {
    ...
    beginner:{
      ...
      theory:"[IS]\nDEMAND FACTORS BY LOAD TYPE (IS 732 / NBC 2016):\nLIGHTING:\nAll circuit types: DF = 1.0\nWarehouse lighting: DF = 0.7\n\nGENERAL POWER SOCKETS:\nOffice power sockets: DF = 0.4–0.5\nRetail power points: DF = 0.6\nKitchen equipment: DF = 0.65\n\nAIR CONDITIONING:\nSplit units residential: DF = 0.5–0.6\nCentral HVAC chiller: DF = 0.85\n\nMOTORS:\nPumps/fans (HVAC): DF = 0.75\nIndustrial motors: DF = 0.7–0.8\n\nIT EQUIPMENT:\nServer loads: DF = 0.9–1.0\n\n[NEC]\nDEMAND FACTORS BY LOAD TYPE (NEC Art 220):\nLIGHTING (NEC 220.42):\nHospitals/Hotels: First 50,000 VA at 40%, remainder at 20%.\nOffices/Commercial: 100% of calculated load.\n\nRECEPTACLES (NEC 220.44):\nNon-dwelling: First 10 kVA at 100%, remainder at 50%.\n\nKITCHEN EQUIPMENT (NEC 220.56):\nCommercial (1-2 items): 100%\nCommercial (6+ items): 65%\n\nMOTORS (NEC 430.24):\nHighest rated motor at 125%, plus 100% of all other motors.\n\n[IEC]\nSIMULTANEITY FACTORS (IEC 60364-1 / Local Utility):\nLIGHTING:\nGeneral commercial: ku = 0.9\n\nSOCKET OUTLETS:\nGeneral commercial: ku = 0.1 to 0.2\n\nHVAC & MOTORS:\nMotors (group): ku = 0.7 to 0.8\n\nDATA/IT EQUIPMENT:\nServers: ku = 1.0",
      formula:{
...

  {
    ...
    beginner:{
      ...
      theory:"[NEC]\nNEC LANGUAGE HIERARCHY:\n1. 'SHALL' = Mandatory requirement. No choice.\n2. 'SHALL NOT' = Absolute prohibition.\n3. 'SHOULD' = Recommendation (not code-enforceable, but good practice).\n4. 'MAY' = Permission.\n5. Informational Note = Explanation only. NOT enforceable.\n6. Exception = Modifies the base requirement.\n\nFINDING THE RIGHT CLAUSE:\nStep 1: Identify the subject (cable? breaker?)\nStep 2: Identify the Chapter (wiring → Ch.2, equipment → Ch.4)\nStep 3: Find the Article number\nStep 4: Read the base rule\nStep 5: Check for Exceptions\nStep 6: Check if Chapter 5, 6, or 7 modifies this Article\n\nCONFLICT RESOLUTION:\nCode takes precedence over standards. When code and client spec conflict: more stringent applies.\n\n[IS]\nIS STANDARD LANGUAGE:\n'SHALL' = Mandatory.\nNotes and Annexures = Informational unless legally adopted or cited as mandatory.\n\nIS STANDARD CROSS-REFERENCES:\nIS standards often reference each other:\n— IS 732 references IS 3043 for earthing\n— IS 3961 is referenced by IS 732 for cable ratings\n— NBC 2016 Part 8 references multiple IS standards\nAlways follow the reference chain completely.\n\nCONFLICT RESOLUTION:\nWhen IS and IEC conflict: IS takes precedence in India.\nWhen IS and client specification conflict: more stringent requirement applies.\nWhen CEA Regulations conflict with IS: CEA Regulations are legally binding.\n\n[IEC]\nIEC LANGUAGE HIERARCHY:\n'SHALL' = Mandatory requirement.\n'SHOULD' = Recommended.\n'MAY' = Permitted.\nNotes = Always informational.\n\nFINDING THE RIGHT CLAUSE (IEC 60364):\nPart 1: Fundamental principles\nPart 4: Protection (overcurrent, shock)\nPart 5: Selection of equipment\nPart 6: Verification\nPart 7: Special installations (hospitals, marinas)\n\nCONFLICT RESOLUTION:\nWhen national laws and IEC conflict: National laws take precedence.\nWhen IEC and client spec conflict: more stringent requirement applies.",
...

  {
    ...
    beginner:{
      ...
      theory:"[[CONDUIT FILL (IS / IEC):|CONDUIT FILL (NEC):]]\nThe percentage of conduit cross-sectional area occupied by cables.\n\n[[FILL LIMITS (General convention):|FILL LIMITS per NEC Chapter 9 Table 1:]]\n— 1 conductor: 53% of conduit area\n— 2 conductors: 31% of conduit area\n— 3 or more conductors: 40% of conduit area\n\n[[IS / IEC approach: similar principle but expressed as number of cables per conduit size.|NEC approach strictly enforced via cross-sectional area limitations.]]\n\nCABLE TRAY FILL:\nFor ladder-type tray: cables fill to maximum depth = 1.5× cable diameter.\nFor solid-bottom tray: maximum fill ratio 50% of tray cross-section area.\nCable tray fill also subject to weight per unit length limit.\n\n[[CONDUIT SIZES (metric, IS):|CONDUIT SIZES (trade size, NEC):]]\n[[16mm, 20mm, 25mm, 32mm, 40mm, 50mm, 63mm internal dia|½\", ¾\", 1\", 1¼\", 1½\", 2\", 2½\" internal diameter]]\n\n[[CABLE TRAY WIDTHS (standard):|CABLE TRAY WIDTHS (standard US):]]\n[[50, 75, 100, 150, 200, 300, 450, 600mm width|6\", 9\", 12\", 18\", 24\", 30\", 36\" width]]\n[[Depth: 25, 50, 75, 100mm|Depth: 3\", 4\", 5\", 6\"]]",
      formula:{
